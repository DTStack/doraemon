...
test('主机管理等四个低频入口被收纳到更多菜单', () => {
    ...
    assert.ok(moreMenu);
    assert.equal(moreMenu.children.length, 4);
    assert.deepEqual(
        moreMenu.children.map((item) => item.path),
        [
            '/page/host-management',
            '/page/env-management',
            '/page/config-center',
            '/page/tags',
        ]
...
test('子菜单点击时使用顶层更多菜单作为选中项', () => {
    assert.equal(resolveMenuClickKey({ key: '/page/tags', keyPath: ['/page/tags', MORE_MENU_PATH] }), MORE_MENU_PATH);
    assert.equal(resolveMenuClickKey({ key: '/page/agents', keyPath: ['/page/agents'] }), '/page/agents');
});
...
test('导航配置不使用可选链语法，兼容当前前端构建链', () => {
    ...
    assert.equal(
        content.includes('?.'),
        false,
        'nav-config.js 不能使用 optional chaining 语法'
    );
});
