...
import { Button, Card, Form, Input, message, Modal, Popconfirm, Switch, Table } from 'antd';
...
function DTDingConfig(props: Props) {
    const { id, type, className, showSwitch } = props;
    ...
    const loadDingTalkList = useCallback(() => {
        API.getConfigNoticeUrlList({ id, type }).then((res: any) => {
    ...
    const handleDelDing = (id: number) => {
        return API.delNoticeUrl({ id, type }).then((res: any) => {
            const { success } = res;
            if (success) {
    ...
    const addDingTalk = (webHook: string, accept_group: string) => {
        API.addConfigNoticeUrl({ id, webHook, type, accept_group }).then((response) => {
    ...
    const checkReapt = (rule: any, value: string, callback: Function) => {
        dingTalkList.forEach(({ accept_group }) => {
    ...
    return (
        <Card
            ...
            extra={
                showSwitch && <Switch checkedChildren="开" unCheckedChildren="关" defaultChecked />
            }
        >
            ...
            >
                <Form form={form} {...formItemLayout} autoComplete="off">
                    <Item
                        ...
                        validateFirst
                        rules={[
                            { required: true, message: '请输入接收群名称' },
                            { validator: checkReapt },
                        ]}
                    >
