...
import { Form, Input, message as Message, Modal, Radio, Select, Switch } from 'antd';
...
const SubscriptionModal = (props: any) => {
    ...
    useEffect(() => {
        ...
        if (visible && data) {
            const {
                ...
                topicIds = [],
                siteNames = '',
                sendType = SUBSCRIPTIONSENDTYPE.WORKDAY,
                time = '',
                messageTitle = '',
                message = '',
                isAtAll = false,
            ...
            const radio =
                siteNames === '自定义消息' && !!message ? 'custom-message' : 'article-subscription';
    ...
    const updateSubscription = (params: any) => {
        ...
        API[params.id ? 'updateSubscription' : 'createSubscription'](params)
            .then(({ success, msg }) => {
                if (success) {
    ...
    const handleTopicChange = (value, option) => {
        setSiteNames(
            Array.from(new Set(option.map((item) => item.children.split(' - ')[0]))).join('、')
        );
    };
