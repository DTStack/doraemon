...
class ProxyServer extends React.PureComponent<any, any> {
    ...
    getProxyServerList = (_checked?: boolean) => {
        const { mainTableParams, collectTagList } = this.state;
        this.setState({
            mainTableLoading: true,
        });
        API.getProxyServerList(mainTableParams)
            .then((response: any) => {
                const { success, data } = response;
                if (success) {
                    this.setState({
                    ...
                    if (!data?.data?.length && mainTableParams.projectId !== undefined) {
                        localStorage.setItem(
                            'collection-tags',
                            JSON.stringify(
                                collectTagList.filter(
                                    (item) => item.id !== mainTableParams.projectId
                                )
                            )
                        );
                        this.getCollectTagList();
    ...
    handleProxyServerEdit = (row: any) => {
        ...
        // 获取关联的目标服务地址列表数据
        this.getTargetAddrs(row.id, (data: any) => {
    ...
    handleProxyServerModalOk = (proxyServer: any) => {
        ...
        API[proxyServer.id ? 'updateProxyServer' : 'addProxyServer']({
            ...
            if (success) {
                Message.success(
                    `${JSON.stringify(currentProxyServer) === '{}' ? '新增' : '编辑'}代理服务成功`
                );
                this.handleProxyServerModalCancel();
                this.getProxyServerList();
                if (expandedRowKeys.length) {
                    this.getTargetAddrs(expandedRowKeys[0], (data: any) => {
            ...
            onOk: () => {
                API.deleteProxyServer({
                    ...
                    if (success) {
                        ...
                        localStorage.setItem(
                            'collection-tags',
                            JSON.stringify(collectTagList.filter((item) => item.id !== id))
                        );
                        this.getCollectTagList();
                        this.getProxyServerList();
                    }
    ...
    onHandleUpdateProxyRule = (record: any) => {
        ...
        const regResult = urlReg.exec(target);
        let tagetIp = target;
        if (regResult) {
            const tagetArrIPs = regResult[2].split(':') || [];
            tagetIp =
                tagetArrIPs[0] != ip
                    ? target
                    : `${regResult[1]}://${localIp}:${tagetArrIPs[1] || '8080'}`;
        }
        API.updateProxyRule({
            id,
    ...
    setCollectTag = (row: any, collected: any) => {
        ...
        if (collected) {
            ...
            const start =
                collectTagList.length > collectMax ? collectTagList.length - collectMax : 0;
    ...
    tableExpandedRowRender = (mainTableRow: any) => {
        ...
        const columns: any = [
            ...
            },
            {
                ...
                render: (text: any) => {
                    return text === localIp ? (
                        <Tooltip placement="topLeft" title={`当前用户 IP:${localIp}`}>
                            <span className="ip-wrap">
                                <i className="iconfont iconicon_star" />
                                {text}
                            </span>
                        </Tooltip>
                    ) : (
                ...
                render: (text: any, record: any) => {
                    return (
                        <Switch
                            ...
                            checkedChildren="开"
                            unCheckedChildren="关"
                            onChange={(e: any) => this.handleChangeStatus(e, record)}
                        />
                    );
                },
            },
                    ...
                    return (
                        <React.Fragment>
                            ...
                            <Popconfirm
                                placement="right"
                                title="确认是否删除该代理规则"
                                onConfirm={this.handleProxyRuleDelete.bind(this, row, mainTableRow)}
                            >
                                <a>删除</a>
                            </Popconfirm>
                        </React.Fragment>
        ...
        return (
            <div style={{ padding: '0 10px' }}>
                <div className="text-right marginBottom12">
                    ...
                    <Switch
                        className="all-ip-switch"
                        checkedChildren="全部"
                        unCheckedChildren="我的"
                        checked={allIPChecked}
                        onChange={(checked) => {
                            this.handleIPFilter(checked, mainTableRow);
                        }}
                    />
                </div>
                <Table
                    ...
                    dataSource={
                        allIPChecked
                            ? subTableData
                            : subTableData.filter((item) => item.ip === localIp)
                    }
                    pagination={false}
                />
            </div>
    ...
    handleIPFilter = (allIPChecked, mainTableRow) => {
        const { localIp } = this.state;
        localStorage.setItem(
            `${localIp}-${mainTableRow.id}-all-ip-checked`,
            allIPChecked ? 'yes' : 'no'
        );
        ...
        const columns: any = [
            ...
            },
            {
                ...
                render: (value: any, record: any) => (
                    <span>
                        {value}
                        {record.api_doc_url && (
                            <a href={record.api_doc_url} rel="noopener noreferrer" target="_blank">
                                &nbsp;[接口文档]
                            </a>
                        )}
                    </span>
          ...
          <a onClick={this.handleProxyServerStatusChange.bind(this, row)}>{Boolean(status) ? '禁用' : '重启'}</a> */}
                            ...
                            <Tooltip title="设置为常用项目">
                                <a onClick={() => this.setCollectTag(row, collected)}>
                                    {collected ? '取消收藏' : '收藏'}
                                </a>
                            </Tooltip>
                        </React.Fragment>
            ...
            <div className="page-proxy-server">
                <div className="title_wrap">
                    <div className="title-left">
                        ...
                        />
                        {collectTagList.length ? (
                            <span style={{ marginRight: 8, marginLeft: 20, lineHeight: '32px' }}>
                                常用项目:
                            </span>
                        ) : null}
                ...
                {config.proxyHelpDocUrl && (
                    <img
                        className="help-icon"
                        src={helpIcon}
                        onClick={this.handleHelpIcon}
                        alt="帮助文档"
                    />
                )}
