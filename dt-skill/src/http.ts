...
export function createHttpClient(options: HttpClientOptions = {}): HttpClient {
    ...
    async function fetchBinaryRequest(
        registry: string,
        args: TextRequestArgs
    ): Promise<Uint8Array> {
        const url = 'url' in args ? args.url : registryUrl(args.path, registry).toString();
        return await runWithRetries(async () => {
            if (deps.runtime === 'bun') {
    ...
    async function downloadZipRequest(registry: string, args: { slug: string; version?: string }) {
        ...
        return await runWithRetries(async () => {
            ...
            const headers: Record<string, string> = {};
            const response = await fetchWithTimeout(deps, url.toString(), {
                method: 'GET',
                headers,
            });
            if (!response.ok) {
...
function getRetryDelayMs(attemptError: unknown, random: () => number): number {
    ...
    if (
        rootError instanceof HttpStatusError &&
        rootError.rateLimit.retryAfterSeconds !== undefined
    ) {
        return (
            rootError.rateLimit.retryAfterSeconds * 1000 + jitterMs(RETRY_AFTER_JITTER_MS, random)
        );
    }
...
async function fetchJsonFormViaCurl(
    ...
    try {
        ...
        } = parseCurlBodyAndMeta(result.stdout ?? '');
        if (status < 200 || status >= 300) {
            throwHttpStatusError(status, body, responseHeaders, deps.now);
        }
        return JSON.parse(body || 'null') as unknown;
...
async function fetchTextViaCurl(deps: Pick<HttpClientDeps, 'spawnSyncImpl' | 'now'>, url: string) {
