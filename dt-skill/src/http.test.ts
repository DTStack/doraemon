...
describe('detectHttpRuntime', () => {
    it('detects bun and node runtimes explicitly', () => {
        expect(detectHttpRuntime({ bun: '1.2.3' } as unknown as NodeJS.ProcessVersions)).toBe(
            'bun'
        );
        expect(detectHttpRuntime({ node: '22.0.0' } as unknown as NodeJS.ProcessVersions)).toBe(
            'node'
        );
    });
});
...
describe('node http client', () => {
    ...
    it('posts form data, retries 429, and uses the upload timeout', async () => {
        ...
        const successClient = createNodeClient({
        ...
        const retryClient = createNodeClient({
            ...
            setTimeoutImpl: createImmediateTimeouts()
