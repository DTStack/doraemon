...
export async function cmdInspect(opts: GlobalOpts, slug: string, options: InspectOptions = {}) {
    ...
    try {
            ...
            versionResult = await apiRequest(
                registry,
                {
                    ...
                    path: `${ApiRoutes.skills}/${encodeURIComponent(
                        trimmed
                    )}/versions/${encodeURIComponent(targetVersion)}`,
                },
                ApiV1SkillVersionResponseSchema
            );
        }
        ...
        if (options.file) {
            const url = registryUrl(
                `${ApiRoutes.skills}/${encodeURIComponent(trimmed)}/file`,
                registry
            );
        ...
        const shouldPrintMeta =
            !options.file || options.files || options.versions || options.version;
        if (shouldPrintMeta) {
            printSkillSummary({
                skill,
                latestVersion: skillResult.latestVersion,
                versionLicense:
                    (versionResult?.version as { license?: string | null } | undefined)?.license ??
                    null,
                owner: skillResult.owner,
            });
...
function printSkillSummary(result: {
    ...
    console.log(
        `License: ${
            result.versionLicense ?? result.latestVersion?.license ?? PLATFORM_SKILL_LICENSE
        } (${PLATFORM_SKILL_LICENSE_SUMMARY})`
    );
    const tags = normalizeTags(skill.tags);
