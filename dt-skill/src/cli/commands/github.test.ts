...
describe('github publish source helpers', () => {
    ...
    it('prefers an existing local path from an alternate workdir', async () => {
        ...
        try {
            ...
            await expect(
                resolveSourceInput('.', {
                    workdir: workspace,
                    localWorkdirs: [callerCwd, workspace],
                })
            ).resolves.toEqual({
                kind: 'local',
                path: callerCwd,
    ...
    it('extracts GitHub archives that contain explicit directory entries', async () => {
        const archiveBytes = zipSync({
            ...
            'repo-root/package.json': new TextEncoder().encode(
        ...
        try {
            ...
            expect(await readFile(join(fetched.dir, 'package.json'), 'utf8')).toContain(
                '"name":"demo"'
            );
        } finally {
